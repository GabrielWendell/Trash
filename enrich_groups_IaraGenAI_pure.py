...
# -*- coding: utf-8 -*-
"""
EVA Groups → Enriched JSON per group (PURE IaraGenAI, no heuristics)
--------------------------------------------------------------------

This script scans the groups tree and, for each group, emits a JSON with
enriched agent fields by cross‑matching agent YAMLs with EVA logs.

**Changes vs `enrich_groups.py`**
- Removed all heuristic/placeholder summarizers.
- Descriptions are generated **exclusively** by IaraGenAI
  (`mrm_copilot.core.agents.Agent`) using a fixed PT‑BR template.
- If Iara fails/returns empty, a clear placeholder is written; there is **no
  heuristic fallback**.

CLI example
    python enrich_groups_IaraGenAI_pure.py \
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Enrich EVA group agents by cross-matching YAMLs with logs (Iara-only descriptions)")
...
# -------------------- Iara-only Description Generator -----------------

class IaraOnlyDescriptionGenerator:
    """Itaú IaraGenAI wrapper (no heuristics, no fallback besides placeholder).

    - Uses a fixed **PT‑BR** system prompt enforcing the one‑sentence template.
    - Sends the agent YAML `prompt` as the user `content`.
    - Retries + polite sleep between calls.
    - If all attempts fail or answer is empty → returns a clear placeholder.
    """
...
def main() -> None:
    ...
    for group_id, DF_AG, DF_METR, owner, members, dom_model in per_group_outputs:
        ...
        for _, ag in DF_AG.iterrows():
            ag_id = str(ag["id"])
            # Iara‑only description (no heuristics)
