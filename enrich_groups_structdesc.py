...
# -*- coding: utf-8 -*-
"""
EVA Groups → Enriched JSON per group (GLOBAL scope compatible) with structured descriptions
-----------------------------------------------------------------------------------------

This variant of `enrich_groups_global_fixed.py` keeps the same behavior but
replaces the free-text prompt summary with a structured, template-based
description:
...
The extraction is heuristic (regex-based) and requires no external LLM.

Usage (example):
    python enrich_groups_structdesc.py \
      --groups-root "Agentes_Chatbots/s3_agents_download/groups" \
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Enrich EVA group agents by cross-matching YAMLs with logs (structured descriptions)")
