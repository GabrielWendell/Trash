...
# -*- coding: utf-8 -*-
"""
EVA Groups → Enriched JSON per group (GLOBAL scope fix)
------------------------------------------------------

This version fixes the crash when running with --score-scope global caused by
duplicate agent ids across different groups. In global mode, multiple rows in
`global_scores` can share the same agent id (one for each group). When the code
later did `float(DF_SCO.loc[ag_id, "score"])`, Pandas returned a Series, which
is not directly convertible to float.

Fix: in the writer loop, when --score-scope global, we subset `global_scores`
by the current `group_id` before looking up scores, making the index unique.

Other robustness features retained from the previous build:
- tolerant YAML parsing
- landing/model/selected_agent filters; robust timestamp parsing
- deterministic prompt summary stub
- dominant model selection with recency tie-breaker
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Enrich EVA group agents (global scope fix)")
...
def main() -> None:
    ...
    DF, _drop_stats = policy_filter(DF, verbose=args.verbose)
    ...
    for group_dir in sorted([d for d in groups_root.iterdir() if d.is_dir()]):
        ...
        agents_list = [str(a).strip() for a in G.get("agents", []) if str(a).strip()]
        ...
        if group_agents_dir.exists():
            for yml in sorted(group_agents_dir.glob("*.yaml")):
                ...
                try:
                    ...
                    tmp = None
                rows.append({"id": ag_id, "name": " ".join(ag_nm.split()), "prompt": pr, "temperature": tmp})
        ...
        # Compute per-agent metrics and dominant model
        ...
        for _, ag in DF_AG.iterrows():
            ...
            if mask_id.any():
                ...
                rows_df = DF.loc[DF["selected_agent"].str.casefold() == ag_nm.casefold()] if ag_nm else DF.iloc[0:0]
            ...
            if len(rows_df) == 0:
                ...
                hhi = 1.0 if tot <= 0.0 else float(((per_user_w / tot).astype(float) ** 2).sum())
        ...
        DF_METR = (pd.DataFrame(metr_rows).set_index("id") if metr_rows
                   else pd.DataFrame({
                        "id": DF_AG["id"],
    ...
    for group_id, DF_AG, DF_METR, owner, members, dom_model in per_group_outputs:
        if args.score_scope == "per-group":
            ...
            # IMPORTANT: slice by this group to avoid duplicate index ambiguity
            DF_SCO = global_scores[global_scores["__group_id__"] == group_id].drop(columns=["__group_id__"]).copy()

        # ensure index is unique for safe lookup
        if not DF_SCO.index.is_unique:
            DF_SCO = DF_SCO[~DF_SCO.index.duplicated(keep="first")]

        records: List[Dict[str, object]] = []
        for _, ag in DF_AG.iterrows():
            ag_id = str(ag["id"]) ; desc = summarizer.summarize(str(ag.get("prompt", "")))
            score_val = 0.0
            if ag_id in DF_SCO.index:
                val = DF_SCO.loc[ag_id, "score"]
                # val could still be a Series in pathological cases; be defensive
